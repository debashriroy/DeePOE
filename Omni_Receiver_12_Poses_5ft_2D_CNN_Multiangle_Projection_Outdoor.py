...
sample_size = 512 # CHANGE AND EXPERIMENT
no_of_samples = 8000 # CHANGE AND EXPERIMENT
...
xdata, ydata = xydata[:,0:sample_size*no_of_features], xydata[:,((sample_size*no_of_features))]   # THE LAST COLUMN IS THE YDATA #  USE 2 INSTEAD OF 8 OF YOU DO NOT USE MULTI-ANGULAR PROJECTION
...
xtrain, xtest, ytrain, ytest = train_test_split(xdata, ydata, test_size=0.2, random_state=42)  # 80/20 is train/test size
...
num_classes = 12  # TOTAL NUMBER OF Poses
...
def plot_confusion_matrix(cm, title='Confusion Matrix', cmap=plt.cm.YlGnBu, labels=[], normalize=False):
    ...
    plt.tight_layout()
    fig.savefig('/Users/debashri/Desktop/DirectionFinding_Plots/Outdoor/5ft/direction_12_poses_conf_mat_2D_CNN_Mapping.png')  # save the figure to file
    plt.close(fig)
